...
double amp=10000.;
static int ninterp;
int sine_func(short *data,int strt_count,int n_data,double amp, double f, int id);
...
LRESULT CALLBACK WindowProc(HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        for(k=0; k<kNumBuffers; k++) {
            ...
            rdpara=sine_func(lpWave[k],rdpara,n_data,amp, f, 0); //波形データ作成
        ...
        if(((LPWAVEHDR)lParam)->dwUser) { //waveOutReset関数で停止
            ...
            for(k=0; k<kNumBuffers; k++) {
                if((LPWAVEHDR)lParam==&whdr[k]) {
                    ...
                    k0++;
                    rdpara=sine_func(lpWave[buf_num],rdpara,n_data,amp, f,1);
                    break;
                }
            }
