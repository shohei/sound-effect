/* ------------------------------------------------------------------
   ...
   * the difference from fsread :
     1) Initiallizing process is completely different. L is required.
     2) Original version does not have final processes. and *nleft is
        not used as an argument.


    Coded by Naotoshi Osaka        12/21/1994
   ------------------------------------------------------------------ */
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
int fsreadpv(short sbuf[],double rbuf[],int smsec,double sf,int nfft,
             int *nleft,int L,int Q,FILE *input,int index)
{   int i,ie,nshift,rvalue,ntotal,ndata,R,nsize;
    R=nshift=(int)((double)smsec*sf);   /* R and nshift are equivalent */
    nsize= 2*L*nfft+1;
...
main()
{
short sbuf[5],sbbuf[123]; double rbuf[NFFTP1];
int nfft,smsec,index,i,j,ie; double sf;
FILE *input; int fsread();

nfft=NFFTP1-1;smsec=5; sf=1.0;
...
printf("index=0\n");
  ie= fsread(sbuf,rbuf,smsec,sf,nfft,input,0);
for(i=1;i<30;i++){
  ie= fsread(sbuf,rbuf,smsec,sf,nfft,input,i);
  printf("i=%d\n  ",i);
