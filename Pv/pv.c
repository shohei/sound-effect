/* ---------------------------------------------------------------
   Phase Vocoder by Portnoff's algorithm
          Copyright (c) 2012 Naotoshi Osaka 2012
   --------------------------------------------------------------- */
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
//  See "pv.h" for the definition of NFFTP or other Capital letter variables.
  ...
  printf("nshift=%d\n",nshift);
    ...
    for(i=index=ndata=iposn=0; ; i++) { /* --- Big loop for frame processing ------ */
        ...
        if((ie1=fft(nfft,wr0buf,wibuf[prpos]))<0) {erout(4); return(0);}
        /* pwr */
        for(j=0; j<=nfft/2; j++)
        ...
        /* (5) */
         if((ie1=fft(-nfft,wrbuf[prpos],wibuf[prpos]))<0){erout(4); return(0);}
        /*   if(i>=0){
    ...
    for(i=nframe=0;; i++) {
        if((ie =fread(sbuf,sizeof(short),nshift,input))<0)   {erout(5); return(0);}
        if((ie1=fread(synbuf,sizeof(short),nshift,input1))<0){erout(5); return(0);}
        for(j=0; j<ie; j++) {
